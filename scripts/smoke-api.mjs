...
async function main() {
  ...
  const happyPhone = `+7999${String(startedAt).slice(-7)}`;
  const happyRequisite = await createRequisite(happyPhone, happyTrader.id);
  ...
  await closeShiftRequisite(happyShiftRequisite.id, 100000, 50000);
  await uploadCSV("trader", "inbound", traderCSV(happyTrader.externalWorkerName, "smoke-in-ok", 1000, "hand_success", happyPhone));
  await uploadCSV("trader", "outbound", traderCSV(happyTrader.externalWorkerName, "smoke-out-ok", 500, "hand_success", happyPhone));
  ...
  const mismatchPhone = `+7988${String(startedAt).slice(-7)}`;
  const mismatchRequisite = await createRequisite(mismatchPhone, mismatchTrader.id);
  ...
  await closeShiftRequisite(mismatchShiftRequisite.id, 200000, 50000);
  await uploadCSV("trader", "inbound", traderCSV(mismatchTrader.externalWorkerName, "smoke-in-mismatch", 1000, "hand_success", mismatchPhone));
  await uploadCSV("trader", "outbound", traderCSV(mismatchTrader.externalWorkerName, "smoke-out-matched", 500, "hand_success", mismatchPhone));
...
async function createRequisite(phone, assignedTraderId) {
  const proxySuffix = phone.replace(/\D/g, "");
...
async function closeShiftRequisite(shiftRequisiteId, inboundTurnoverMinor, outboundTurnoverMinor) {
  await request(`/trader/shift-requisites/${shiftRequisiteId}/close`, {
    method: "POST",
    body: {
      inboundTurnoverMinor,
      outboundTurnoverMinor,
      closingBalanceMinor: 0,
      blocked: false,
      comment: "Smoke close requisite",
    },
  });
}
...
function traderCSV(workerName, innerId, amount, status, phone) {
  const requisitePhone = phone.replace(/\D/g, "");
  return [
    ...
    `${innerId}|${innerId}|${requisitePhone}|${requisitePhone}|device-1|СБП|sbp|${amount}.0|91.20|RUB|${status}|28.05.2026 14:00:00|28.05.2026 14:05:00|28.05.2026 14:05:00|None|None|smoke|req-1|${workerName}|${amount}.0|0.0|true|true|SM`,
    "",
  ].join("\n");
}
