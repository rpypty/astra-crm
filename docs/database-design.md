# Database Design Draft — PostgreSQL

This is a schema draft. It should be converted into goose migrations and sqlc queries.

## 1. General conventions

- Primary keys: `bigserial` or UUID. For MVP choose one consistently.
- Money: `bigint` minor units preferred.
- Dates: `timestamptz` for system timestamps.
- External CSV dates: store as `timestamptz` if timezone is known, otherwise `timestamp` + documented timezone assumption.
- Statuses: text with CHECK constraints or PostgreSQL enums. For MVP, text + CHECK is flexible.
...
CREATE TABLE trader_profiles (
    ...
    salary_rate_bps BIGINT NOT NULL DEFAULT 0,
...
If multi-team duplicate `workerName` is possible later, use:

```sql
UNIQUE (team_id, external_worker_name)
```

by adding `team_id` to the table.
...
Only one active assignment per requisite:
...
Only one open shift per trader:
...
```sql
CREATE UNIQUE INDEX uq_shift_requisite_once
...
## shift_requisite_internal_transfers
...
Internal transfers are not `external_orders`, `manual_payout_orders`, or `manual_payout_transfers`.
...
Business invariant `sum(transfers) <= order.amount` should be enforced in service transaction, possibly with row lock on payout order.
...
CREATE INDEX idx_order_scope_external_order
ON order_scope_items(external_order_id);
```

Avoid a unique index on `external_order_id` because same order can be present in both trader and teamlead scopes.
...
CREATE TABLE reconciliation_runs (
    ...
    success_amount_minor BIGINT NOT NULL DEFAULT 0,
    success_count BIGINT NOT NULL DEFAULT 0,
    failed_amount_minor BIGINT NOT NULL DEFAULT 0,
...
```text
reassign requisite
start/take shift requisite
add payout transfer + update payout status
CSV import/reimport/apply/reconcile
accept mismatch
close shift
```
...
```text
trader_shifts when closing/reconciling
manual_payout_orders when adding transfers
import scope rows when replacing active scope
```
...
- Import batches and import rows are historical and should not be deleted by normal user actions.
- Audit logs are append-only.
- Reconciliation runs can be append-only; API may show latest run by scope/type.
- Domain entities should use soft delete where history exists.
