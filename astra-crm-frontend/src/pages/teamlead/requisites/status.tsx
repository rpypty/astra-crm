...
import type { RequisiteReportTarget } from "./types";

export function workRowReportTarget(row: RequisiteAssignmentWorkRow): RequisiteReportTarget {
  return {
    id: row.requisiteId,
    phone: row.phone,
...
export function ReportStatusButton({
  ...
  status: string;
  target: RequisiteReportTarget;
  onOpen: (target: RequisiteReportTarget) => void;
}) {
  return (
    <button
      type="button"
      className="inline-flex rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      title="Открыть отчет по реквизиту"
      onClick={(event) => {
