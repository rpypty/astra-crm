...
-- name: BatchInsertImportRows :many
INSERT INTO import_rows (
    import_batch_id,
    row_number,
    external_id,
...
SELECT
    ...
    'parsed',
    NULL
FROM jsonb_to_recordset(sqlc.arg(rows_json)::jsonb) AS input(
    row_number bigint,
    external_id text,
...
-- name: BatchUpsertExternalOrders :many
WITH upserted AS (
...
-- name: CreateTraderShiftScopeItemsBatch :many
...
SELECT
    ...
    TRUE
FROM import_rows
JOIN external_orders ON external_orders.team_id = sqlc.arg(team_id)
    AND external_orders.direction = sqlc.arg(direction)
    AND external_orders.external_inner_id = import_rows.external_inner_id
WHERE import_rows.import_batch_id = sqlc.arg(import_batch_id)
ORDER BY import_rows.row_number
RETURNING id;

-- name: CreateTeamleadPeriodScopeItemsBatch :many
INSERT INTO order_scope_items (
    team_id,
    scope_type,
    direction,
